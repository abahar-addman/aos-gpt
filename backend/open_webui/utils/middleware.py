...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    if payload_tools is None:
        ...
        if tools_dict:
            ...
            if function_calling_mode == 'langgraph':
                # LangGraph agent mode: recursive tool calling with analysis between
                # iterations. The agent loop owns the think -> call -> analyse cycle
                ...
                try:
                    ...
                    # Fall back to non-native tool handling
                    try:
                        ...
                        log.exception(e2)
            elif function_calling_mode != 'legacy':
            ...
            try:
                ...
                while tool_calls and (
                    ...
                    async def execute_tool_call(tool_call):
                        ...
                        # Edison: announce the call before it runs. v0.11.0 hoisted tool
                        # execution out of the results loop (delegate_task calls now run
                        # concurrently), so the status has to be emitted here to still
                        # precede the work rather than report it after the fact.
                        await event_emitter(
                            {
                                'type': 'status',
                                'data': {
