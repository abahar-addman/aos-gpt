...
def save_docs_to_vector_db(
    ...
    if split:
        ...
        if config.TEXT_SPLITTER in ['', 'character']:
            ...
            log.debug(f'Using token text splitter: {config.TIKTOKEN_ENCODING_NAME}')
    ...
    try:
        if VECTOR_DB_CLIENT.has_collection(collection_name=collection_name):
            ...
            if overwrite:
                ...
                log.debug(f'collection {collection_name} already exists, overwrite is False and add is False')
                return True
