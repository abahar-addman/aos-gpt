...
async def ldap_auth(
    ...
    try:
        ...
        if ENABLE_LDAP_GROUP_MANAGEMENT:
            ...
            log.debug(f'LDAP Group Management enabled. Adding {LDAP_ATTRIBUTE_FOR_GROUPS} to search attributes')
        ...
        if ENABLE_LDAP_GROUP_MANAGEMENT and LDAP_ATTRIBUTE_FOR_GROUPS in entry:
            ...
            if group_dns:
                ...
                log.debug(f'LDAP group_dns after processing - type: {type(group_dns)}, length: {len(group_dns)}')
                ...
                log.debug(f'LDAP groups for user {username_list}: {user_groups} (total: {len(user_groups)})')
            else:
            ...
            if user:
                if ENABLE_LDAP_GROUP_MANAGEMENT and user_groups:
                    try:
                        ...
                        log.debug(f'Successfully synced groups for user {user.id}: {user_groups}')
                    except Exception as e:
...
async def signup_handler(
    ...
    *,
    ...
    if await Users.get_num_users(db=db) == 1:
        await Users.update_user_role_by_id(user.id, 'admin', db=db)
        user = await Users.get_user_by_id(user.id, db=db)
        # NOTE: Do not auto-disable signup after the first user. Edison uses open
        # self-registration gated by SIGNUP_ALLOWED_EMAIL_DOMAIN, so signup must
        # stay enabled for subsequent @addmangroup.com users.
