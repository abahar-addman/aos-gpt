...
async def speech(request: Request, user=Depends(get_verified_user)):
    ...
    # Cache the freshly generated audio to Redis for cross-worker reuse.
    try:
        async with aiofiles.open(file_path, 'rb') as f:
            await redis_cache_set(redis, redis_speech_key, await f.read(), ttl=86400)
    except Exception as exc:
        log.debug(f'Failed to cache speech audio to Redis: {exc}')
...
async def _transcribe_whisper(request, file_path, languages, file_dir, id):
    if request.app.state.faster_whisper_model is None:
        request.app.state.faster_whisper_model = await asyncio.to_thread(
            set_faster_whisper_model, await Config.get('audio.stt.whisper_model'), WHISPER_MODEL_AUTO_UPDATE
        )
...
async def transcription_handler(request, file_path, metadata, user=None):
    ...
    if await Config.get('audio.stt.engine') == '':
        data = await _transcribe_whisper(request, file_path, languages, file_dir, id)
    elif await Config.get('audio.stt.engine') == 'openai':
        data = await _transcribe_openai(request, file_path, filename, languages, file_dir, id, user)
    elif await Config.get('audio.stt.engine') == 'deepgram':
        data = await _transcribe_deepgram(request, file_path, languages, file_dir, id)
    elif await Config.get('audio.stt.engine') == 'azure':
        data = await _transcribe_azure(request, file_path, filename, file_dir, id)
    elif await Config.get('audio.stt.engine') == 'mistral':
        data = await _transcribe_mistral(request, file_path, filename, metadata, file_dir, id)
    else:
        return None

    # Edison: cache transcription result in Redis for cross-worker reuse.
    if data is not None:
        try:
            redis_cache_set_json_sync(_sync_redis, f'audio:transcription:{id}', data, ttl=3600)
        except Exception as exc:
            log.debug(f'Failed to cache transcription to Redis: {exc}')
    return data
