    ...
    *,
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f'Ollama: {e}' if str(e) else 'Edison AI: Server Connection Error',
        )
    finally:
        if not streaming:
...
async def verify_connection(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail='Edison AI: Server Connection Error')
    except Exception as exc:
...
async def upload_model(
    ...
    async def file_process_stream():
        ...
        try:
            ...
            log.debug('Uploaded to /api/blobs')
            os.remove(file_path)

            # Stage 4: create the model
            model, _ext = os.path.splitext(filename)
            log.debug(f'Created Model: {model}')
