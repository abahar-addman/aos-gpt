...
async def send_post_request(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=detail if e else "Edison AI: Server Connection Error",
        )
    finally:
        if not streaming:
            await cleanup_response(r, session)
...
async def verify_connection(
    ...
    async with aiohttp.ClientSession(
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail="Edison AI: Server Connection Error"
            )
        except Exception as e:
...
async def get_openai_models(
    ...
    if url_idx is None:
        ...
        try:
            ...
            log.exception(e)
            error_detail = "Edison AI: Server Connection Error"
            if r is not None:
                try:
                    res = r.json()
                    if "error" in res:
