			...
			<AdminSettingSection title={$i18n.t('Content Extraction')} first>
				...
				{:else if RAGConfig.CONTENT_EXTRACTION_ENGINE === 'olm_ocr'}
					<div class="grid grid-cols-1 gap-x-3 gap-y-2.5 sm:grid-cols-2">
						<AdminSettingField
							label={$i18n.t('API Base URL')}
							description={$i18n.t('Ollama endpoint serving the OlmOCR vision model.')}
						>
							<input
								class={inputClass}
								placeholder={$i18n.t('Enter OLM OCR API Base URL')}
								bind:value={RAGConfig.OLM_OCR_API_BASE_URL}
							/>
						</AdminSettingField>
						<AdminSettingField
							label={$i18n.t('Model')}
							description={$i18n.t('Vision model used to transcribe scanned PDF pages.')}
						>
							<input
								class={inputClass}
								placeholder={$i18n.t('Enter OLM OCR Model')}
