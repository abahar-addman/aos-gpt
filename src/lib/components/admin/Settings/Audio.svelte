	...
	<div class="flex-1 min-h-0 overflow-y-auto scrollbar-hover pr-1.5">
		<AdminSettingSection title={$i18n.t('Speech-to-Text')} first>
			...
			{#if STT_ENGINE === 'openai'}
				<div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
					...
					<AdminSettingField
						...
						description={$i18n.t(
							'Leave the API key empty for self-hosted servers that do not require one.'
						)}
					>
						<SensitiveInput
							variant="settings"
			...
			{:else if STT_ENGINE === ''}
				<AdminSettingField
					label={$i18n.t('STT Model')}
					description={$i18n.t('Edison AI uses faster-whisper internally.')}
				>
		...
		<AdminSettingSection title={$i18n.t('Text-to-Speech')}>
			...
			{:else if TTS_ENGINE === 'transformers'}
				<AdminSettingField
					...
					description={$i18n.t('Edison AI uses SpeechT5 and CMU Arctic speaker embeddings.')}
				>
					<input
