	...
	<div slot="content">
		<DropdownMenu className="{className} font-sans text-xs">
			...
			{#if help}
				...
				<!-- Edison AI: upstream's Documentation and Releases links point at
				     docs.openwebui.com and the open-webui GitHub releases. They are
				     intentionally not surfaced here to keep the product branding
				     consistent. v0.11.0 made them admin-only; we keep them hidden. -->
				<!--
				{#if $user?.role === 'admin'}
					<a href="https://docs.openwebui.com" target="_blank">{$i18n.t('Documentation')}</a>
					<a href="https://github.com/open-webui/open-webui/releases" target="_blank">{$i18n.t('Releases')}</a>
				{/if}
				-->
