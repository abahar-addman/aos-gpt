...
<div class=" space-y-1 text-xs pb-safe-bottom">
	...
	<div>
		<Tooltip
			content={$i18n.t(
				'Enable extended thinking/reasoning for supported models. Forces the model to think step-by-step before responding.'
			)}
			placement="top-start"
			className="inline-tooltip"
		>
				...
				<button
					...
					on:click={() => {
						params.extended_thinking =
							(params?.extended_thinking ?? null) === null
								? true
								: params.extended_thinking
									? false
									: null;
					...
					type="button"
				>
					{#if params.extended_thinking === true}
						<span class="ml-2 self-center">{$i18n.t('On')}</span>
					{:else if params.extended_thinking === false}
						<span class="ml-2 self-center">{$i18n.t('Off')}</span>
					{:else}
						<span class="ml-2 self-center">{$i18n.t('Default')}</span>
					{/if}
