<script lang="ts">
	...
	onMount(async () => {
		// `user` is tri-state (undefined = not yet bootstrapped, null = torn down by
		// the 401 handler, object = signed in), so this must stay a falsy test.
		// gotoAuth() preserves the deep link as ?redirect=.
		if (!$user) {
			await gotoAuth();
			return;
		}
